...
const C={navy:"#0D1B2A",navyMid:"#1A2E45",steel:"#2C4A6E",accent:"#E8A020",accentL:"#F5C842",danger:"#C0392B",dangerL:"#FADBD8",warn:"#E67E22",warnL:"#FDEBD0",ok:"#1A7A4A",okL:"#D4EFDF",g100:"#F7F8FA",g200:"#E8ECF0",g300:"#C5CDD6",g500:"#7A8A99",g700:"#3D4F5E",white:"#FFFFFF"};
          ...
          {prev.map(function(entry,i){
            ...
            return <div key={i}>
              <div onClick={function(){setSelEntry(isSel?null:entry);}} style={{display:"grid",gridTemplateColumns:"2fr 70px 110px 60px",padding:"11px 16px",background:isSel?"#F0F4FF":i%2===0?C.white:C.g100,borderBottom:"1px solid "+C.g200,cursor:"pointer"}}>
                ...
                <div style={{fontSize:11,color:r.alertes&&r.alertes.length>0?C.danger:C.ok,fontWeight:700}}>{r.alertes?r.alertes.length:0} ⚠</div>
              </div>
              {isSel&&<div style={{background:"#F0F4FF",padding:"14px 16px",borderBottom:"1px solid "+C.g200}}>
                {r.passif_par_travailleur&&r.passif_par_travailleur.length>0&&<div style={{marginBottom:12}}>
    ...
    <main style={{maxWidth:980,margin:"0 auto",padding:"24px 20px"}}>
          ...
          {configTab==="grille"&&<div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,background:C.g100,borderRadius:8,padding:"10px 14px",marginBottom:12,border:"1px solid "+C.g200}}>
              ...
              <div style={{display:"flex",gap:6}}>
                {[2022,2023,2024,2025,2026].map(function(y){var g=edit.grille||DEF.grille;var isRef=g.anneeReference===y;return <button key={y} onClick={function(){var ng=Object.assign({},g,{anneeReference:y,annee:y});setEdit(function(p){return Object.assign({},p,{grille:ng});});}} style={{background:isRef?C.navy:C.white,color:isRef?C.white:C.navy,border:"2px solid "+(isRef?C.navy:C.g200),borderRadius:6,padding:"5px 10px",cursor:"pointer",fontWeight:isRef?800:600,fontSize:12}}>{y}</button>;})}
              </div>
      ...
      {step==="home"&&<div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:22}}>
          {[
            {v:co.workers,l:"ETP",s:"comptes 2025"},
            {v:(W*36*5).toLocaleString("fr-BE")+" euros",l:"audit retrospectif",s:(W*36).toLocaleString("fr-BE")+" fiches x 5 euros",accent:true},
            {v:(W*12*5).toLocaleString("fr-BE")+" euros/an",l:"abonnement preventif",s:(W*12).toLocaleString("fr-BE")+" fiches/an x 5 euros"},
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
          {[
            {id:"retro",icon:"🔍",label:"Audit Retrospectif",sub:"Calcul du passif",desc:"Quantifie le passif exact par travailleur sur la periode choisie.",border:C.danger,tag:"ONE-SHOT",has:retro.length>0},
            {id:"preventif",icon:"✅",label:"Audit Preventif",sub:"Validation avant emission",desc:"Verifie chaque fiche avant envoi. Identifie corrections necessaires.",border:C.ok,tag:"ABONNEMENT/AN",has:prev.length>0},
            {id:"dashboard",icon:"📊",label:"Vue Consolidee",sub:"Dashboard automatique",desc:"Resultats de tous les audits en un coup d oeil.",border:C.accent,tag:"INCLUS",has:retro.length>0||prev.length>0},
          ].map(function(m){return <div key={m.id} onClick={function(){selMode(m.id);}}
            style={{background:C.white,border:"2px solid "+C.g200,borderRadius:12,padding:"20px 18px",cursor:"pointer",position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:m.border}}/>
            {m.has&&<div style={{position:"absolute",top:10,right:10,width:8,height:8,borderRadius:"50%",background:C.ok}}/>}
            <div style={{fontSize:26,marginBottom:8}}>{m.icon}</div>
            <div style={{color:C.navy,fontWeight:800,fontSize:14,marginBottom:2}}>{m.label}</div>
            <div style={{color:m.border,fontSize:11,fontWeight:700,marginBottom:8}}>{m.sub}</div>
            <div style={{color:C.g700,fontSize:12,lineHeight:1.5,marginBottom:12}}>{m.desc}</div>
            <span style={{background:C.g100,border:"1px solid "+C.g200,borderRadius:4,padding:"2px 8px",fontSize:10,fontWeight:700,color:m.border}}>{m.tag}</span>
          </div>;})}
        </div>
      </div>}
      ...
      {step==="result"&&result&&<div>
        ...
        <div style={{background:C.navy,borderRadius:12,padding:"18px 20px",marginBottom:12,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:14}}>
          <div>
            ...
            <div style={{color:C.white,fontWeight:800,fontSize:17}}>{(result.worker&&result.worker.name)||co.name}</div>
          ...
          <div style={{textAlign:"right"}}>
            <div style={{display:"inline-block",padding:"3px 12px",borderRadius:4,fontWeight:700,fontSize:12,background:result.conformite_globale==="CONFORME"||result.conformite_globale==="PRETE A EMETTRE"?C.okL:result.conformite_globale==="RISQUE MODERE"?C.warnL:C.dangerL,color:result.conformite_globale==="CONFORME"||result.conformite_globale==="PRETE A EMETTRE"?C.ok:result.conformite_globale==="RISQUE MODERE"?C.warn:C.danger}}>{result.conformite_globale}</div>
            <div style={{marginTop:7}}><span style={{color:scoreC(result.score),fontSize:32,fontWeight:900}}>{result.score}</span><span style={{color:C.g300,fontSize:12}}>/100</span></div>
          </div>
        </div>
        {result.risque_financier_estime&&result.risque_financier_estime!=="0 euros - fiche conforme"&&<div style={{background:C.dangerL,border:"1px solid "+C.danger,borderRadius:10,padding:"12px 18px",marginBottom:12,display:"flex",gap:12,alignItems:"center"}}>
